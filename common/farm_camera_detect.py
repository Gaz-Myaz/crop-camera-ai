...
def main() -> int:
    ...
    if args.camera_name:
        ...
        match = next(
            (i for i, n in enumerate(names) if args.camera_name.lower() in n.lower()),
            None,
        )
        if match is None:
            print(f"ERROR: no camera name containing '{args.camera_name}' found.")
            print("Available names:", names or
                  "(none detected -- is 'pygrabber' installed, and are you on Windows?)")
            print("Falling back to --camera index instead, or run --list-cameras.")
            return 1
        camera_index = match
    ...
    try:
        while True:
            ...
            if not args.no_display:
                ...
                cv2.imshow("DECXIN-SM-2930V1 - Object Detection", annotated)
